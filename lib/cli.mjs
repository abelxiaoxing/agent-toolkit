...
async function installManagedWorkflow(options) {
  ...
  const claudeResults = await linkClaude(options.agentsDir, previousLinkedTargets);
  const codexResults = await linkCodex(options.agentsDir, previousLinkedTargets);
...
async function configurePromptEnhancerEnv(agentsDir) {
  ...
  const existing = await readDotenvFile(envPath);
  const mode = await promptSelect("请选择 prompt-enhancer 的运行方式", [
    { value: "openai-compatible", label: "第三方 OpenAI 兼容接口（填写 URL / API Key / Model）" },
    { value: "agent", label: "直接使用当前 Agent，不写第三方配置" }
  ], { defaultValue: resolvePromptEnhancerMode(existing) });

  if (mode === "openai-compatible") {
    const apiUrl = await promptText("PE_API_URL", {
      defaultValue: existing.PE_API_URL || undefined
    });
    const apiKey = await promptSecret("PE_API_KEY", {
